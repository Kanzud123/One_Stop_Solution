    ...
    public class UserController : Controller
    {
       ...
       //[NoDirectAccess]
        ...
        public ActionResult Profile()
        {
            ...
            var profile = " select  top 1 * from tbl_user ORDER BY u_id DESC ";
            List<tbl_user> us2 = db.tbl_user.SqlQuery(profile).ToList();
            return View(us2);
        }
        ...
        {
            tbl_user ad = db.tbl_user.Find(id);
            if (ad == null)
            {
        ...
        [HttpPost]
        public ActionResult Edit(tbl_user ad)
        {
        ...
        public ActionResult Delete(int id)
        {
            if (id > 0)
            {
                var data = db.tbl_product.Where(x => x.pro_fk_user == id).FirstOrDefault();

                db.tbl_product.Remove(data);
                db.SaveChanges();
                var data1 = db.tbl_user.Where(x => x.u_id == id).FirstOrDefault();
        ...
        {
            tbl_product ad = db.tbl_product.Find(id);
            if (ad == null)
            {
        ...
        [HttpPost]
        public ActionResult EditAd(tbl_product ad)
        {
